...
BEGIN
  ...
  -- Lock the relevant cursor before choosing an agent. This makes simultaneous
  -- lead arrivals advance one at a time instead of selecting the same person.
  ...
  SELECT id, last_assigned_overflow_id
    INTO v_overflow_rr_id, v_last_overflow_id
  FROM public.overflow_round_robin_state
  WHERE team_id IS NULL
  LIMIT 1
  FOR UPDATE;
  ...
  IF v_overflow_recipient.admin_user_id IS NOT NULL THEN
    IF v_overflow_rr_id IS NOT NULL THEN
      ...
      SET last_assigned_overflow_id = v_overflow_recipient.id,
          updated_at = v_now
      WHERE id = v_overflow_rr_id;
    ELSE
      INSERT INTO public.overflow_round_robin_state
        (last_assigned_overflow_id, updated_at, team_id)
