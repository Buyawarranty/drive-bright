-- When a recontact lead is claimed/assigned to a new agent, reset its lifecycle
-- status to 'recontact' so the new owner starts on a clean slate instead of
-- inheriting the stale status the previous agent left months ago.
