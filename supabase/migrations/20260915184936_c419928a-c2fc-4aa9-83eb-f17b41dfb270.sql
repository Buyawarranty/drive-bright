...
AS $$
  ...
  SELECT
    ...
    wc.phone,
    LEFT(COALESCE(NULLIF(i.body, ''), CASE WHEN i.media_type IS NOT NULL THEN '[' || i.media_type || ']' ELSE 'New WhatsApp reply' END), 240) AS message_preview,
    i.replied_at
  FROM inbound i
  JOIN public.whatsapp_conversations wc ON wc.id = i.conversation_id
  LEFT JOIN public.sales_leads sl ON sl.id = wc.lead_id
  CROSS JOIN caller c
  WHERE
    c.email = 'support@buyawarranty.co.uk'
    OR wc.assigned_to = c.id
    OR sl.assigned_to = c.id
    OR sl.owner_agent = c.id
    OR c.id = (
      SELECT previous.sent_by_admin_id
      FROM public.whatsapp_messages previous
      WHERE previous.conversation_id = i.conversation_id
        AND previous.direction = 'outbound'
        AND previous.sent_by_admin_id IS NOT NULL
        AND COALESCE(previous.wati_timestamp, previous.created_at) <= i.replied_at
      ORDER BY COALESCE(previous.wati_timestamp, previous.created_at) DESC
      LIMIT 1
    )
  ORDER BY i.replied_at DESC
  LIMIT 100;
$$;
