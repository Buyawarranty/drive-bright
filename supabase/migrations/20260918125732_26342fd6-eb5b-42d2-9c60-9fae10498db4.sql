CREATE OR REPLACE FUNCTION public.open_pool_next_call_slot()
RETURNS timestamptz
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
AS $function$
DECLARE _local timestamp := (now() AT TIME ZONE 'Europe/London'); _d date := _local::date; _slot timestamptz;
BEGIN
  IF extract(isodow from _d) > 5 THEN
    RETURN public.open_pool_next_working_day_9am() + interval '30 minutes';
  END IF;
  IF _local::time < time '09:30' THEN
    _slot := ((_d + time '09:30') AT TIME ZONE 'Europe/London');
  ELSIF _local::time < time '15:30' THEN
    _slot := ((_d + time '15:30') AT TIME ZONE 'Europe/London');
  ELSE
    _slot := public.open_pool_next_working_day_9am() + interval '30 minutes';
  END IF;
  RETURN _slot;
END;
$function$;

REVOKE EXECUTE ON FUNCTION public.open_pool_next_call_slot() FROM anon;
GRANT EXECUTE ON FUNCTION public.open_pool_next_call_slot() TO authenticated, service_role;
...
CREATE OR REPLACE FUNCTION public.open_pool_log_outcome(_lead_id uuid, _agent uuid, _outcome text, _reason text DEFAULT NULL::text, _next_action_at timestamp with time zone DEFAULT NULL::timestamp with time zone)
 ...
 SET search_path TO 'public'
AS $function$
DECLARE _new_status text;_new_queue text;_new_owner uuid;_keep_lock boolean:=false;_tags text[]:=ARRAY[]::text[];_inc_calls integer:=0;_current_calls integer;_attempt_num integer;_lost_tag text;_retry_at timestamptz;_final_action text;_locked_by uuid;_assigned_to uuid;_caller_admin_id uuid;_caller_role text;_is_mgmt boolean:=false;_agent_admin_id uuid;_max_attempts integer:=14;
BEGIN
 ...
 WHEN 'no_answer','voicemail_left','line_busy' THEN
   ...
   ELSE
     -- Cadence: twice a day (mid-morning / mid-afternoon, working days) for 7 days.
     _new_status:='new';_new_queue:='retry_queue';_retry_at:=public.open_pool_next_call_slot();
   END IF;
