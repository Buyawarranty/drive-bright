...
BEGIN
  ...
  IF NOT _no_timer THEN
    ...
    _log_desc := 'Recycled from Open Pool by manager (previously unclaimed lead) — ' || _window_minutes || '-minute call window.';
  ELSE
    _deadline := NULL;
    _new_queue := 'owned_by_agent';
    _log_desc := 'Recycled from Open Pool by manager (previously unclaimed lead) — no time limit (owned by agent).';
  END IF;
  ...
  IF array_length(_ids, 1) IS NOT NULL THEN
    ...
    _note_text := '[System] Recycled from Open Pool — previously unclaimed lead, may be several days old. Check history before calling.';
