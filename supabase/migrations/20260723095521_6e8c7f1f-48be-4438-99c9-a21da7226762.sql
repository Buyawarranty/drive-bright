...
BEGIN
  ...
  -- customer_lock_events is an append-only event log; treat the latest
  -- event per phone as the current state (to_state='locked' == still locked).
  latest_lock AS (
  ...
  double_locks AS (
    -- No released_at column exists; "double locks" was meaningful only with
    -- release tracking. Report 0 rather than failing the whole dashboard.
