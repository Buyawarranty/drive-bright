-- 1) Update the trigger: after dedup-update OR insert, sync cart.contacted_by
--    to the auth.users id that maps to the lead's assigned admin_users.id.
...
BEGIN
  ...
  IF v_derived_source = 'website' THEN
    ...
    ELSIF EXISTS (
      ...
      WHERE lower(btrim(email)) = v_clean_email
        AND cart_metadata IS NOT NULL
        AND (cart_metadata::text LIKE '%fbclid%'
  ...
  IF v_existing_lead_id IS NOT NULL THEN
    ...
    IF v_assigned_auth_uid IS NOT NULL THEN
      NEW.contacted_by := v_assigned_auth_uid;
    END IF;
    RETURN NEW;
  END IF;
  ...
  -- 5) Reset caps + INSERT new lead with round-robin
