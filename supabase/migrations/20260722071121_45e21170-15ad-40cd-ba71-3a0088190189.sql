...
BEGIN
  ...
  agent_state AS (
    SELECT a.agent_id, a.agent_name, a.role,
           ...
           (SELECT COUNT(*) FROM public.customers cu WHERE cu.assigned_to = a.agent_id::text AND cu.created_at > v_now - interval '24 hours' AND cu.status IN ('active','processing','pending_verification')) AS active_sales
    FROM agents a
  )
...
GRANT EXECUTE ON FUNCTION public.orr_queue_dashboard_snapshot() TO authenticated, service_role;
