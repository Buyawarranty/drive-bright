...
BEGIN
  ...
  -- Lowest priority tier with an eligible agent (NULL priority = 999, last resort).
  -- NOTE: assignment_mode filter intentionally removed. Distribution now includes
  -- BOTH Round Robin and Open Pool agents so every active agent fills up one-by-one.
  ...
  IF v_any_on_duty THEN
    -- Fair fill: pick the eligible agent with the FEWEST leads today.
    -- Tie-break by earliest last_assigned_at, then sort_order.
    -- This gives strict one-by-one rotation across ALL active agents
    -- (RR + Open Pool), and once everyone is level, higher-cap agents
    -- naturally continue receiving because low-cap agents hit their cap.
    SELECT adc.* INTO v_next_agent
    FROM public.agent_distribution_caps adc
    JOIN public.admin_users au ON au.id = adc.admin_user_id
    WHERE au.is_active = true
  ...
  -- Overflow (unchanged shape; also drops assignment_mode filter for consistency)
