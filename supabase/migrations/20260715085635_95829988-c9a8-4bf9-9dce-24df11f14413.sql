...
BEGIN
  ...
  -- Priority tiers removed: every eligible agent competes equally in round-robin order.
  SELECT EXISTS (
    SELECT 1
    FROM public.agent_distribution_caps adc
    JOIN public.admin_users au ON au.id = adc.admin_user_id
  ...
  IF v_any_on_duty THEN
    ...
    -- Strict round-robin: everyone gets one before anyone gets two.
    SELECT adc.* INTO v_next_agent
    FROM public.agent_distribution_caps adc
    JOIN public.admin_users au ON au.id = adc.admin_user_id
    WHERE au.is_active = true
...
COMMENT ON COLUMN public.agent_distribution_caps.priority IS
  'Deprecated: no longer used by the assignment picker. Kept for UI/reference. All eligible agents share strict round-robin regardless of priority.';