...
AS $function$
  WITH pool AS (
    ...
    WHERE sl.status NOT IN ('lost','fake_lead','converted','archived')
...
BEGIN
  ...
  WITH picked AS (
    SELECT sl.id
    FROM public.sales_leads sl
    WHERE (sl.assigned_to IS NULL OR sl.assigned_to <> _admin_id)
...
BEGIN
  ...
  WITH picked AS (
    SELECT sl.id
    FROM public.sales_leads sl
    WHERE (sl.assigned_to IS NULL OR sl.assigned_to <> _agent_id)
      AND sl.status NOT IN ('lost','fake_lead','converted','archived')
      AND sl.created_at < now() - _min_age
      AND NOT EXISTS (
        SELECT 1 FROM public.lead_quick_notes lqn
