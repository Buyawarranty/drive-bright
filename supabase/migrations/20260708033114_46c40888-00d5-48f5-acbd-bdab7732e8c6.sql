-- Per-agent assignment mode: round_robin vs open_pool
ALTER TABLE public.agent_distribution_caps
  ADD COLUMN IF NOT EXISTS assignment_mode text NOT NULL DEFAULT 'round_robin'
  CHECK (assignment_mode IN ('round_robin', 'open_pool'));

COMMENT ON COLUMN public.agent_distribution_caps.assignment_mode IS
  'How this agent receives leads: round_robin = auto-assigned by rotation; open_pool = only via Open Pool self-claim.';
