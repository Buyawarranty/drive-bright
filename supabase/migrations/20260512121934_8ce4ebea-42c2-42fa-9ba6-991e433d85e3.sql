...
BEGIN
    ...
    -- OUT OF HOURS → straight to overflow recipients
    ...
    IF v_settings.distribution_mode = 'percentage' AND NOT v_force_overflow THEN
        ...
        IF v_next_agent IS NOT NULL THEN
            ...
            RETURN NEW;
        END IF;
        -- Nobody fits percentage mode → fall through to overflow
        v_force_overflow := true;
    END IF;
    ...
    -- OVERFLOW RECIPIENTS (out-of-hours OR all on-duty agents capped)
