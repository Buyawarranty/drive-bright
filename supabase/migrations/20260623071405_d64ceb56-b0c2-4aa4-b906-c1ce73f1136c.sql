...
BEGIN
  ...
  worked_q AS (
    SELECT worked_rows.worked_agent_id AS agent_id, count(DISTINCT worked_rows.lead_id)::int AS worked_cnt
    FROM (
      SELECT c.new_assigned_to AS worked_agent_id, c.lead_id FROM changelog c WHERE c.new_assigned_to IS NOT NULL
      UNION ALL
      SELECT c.activity_agent_id AS worked_agent_id, c.lead_id FROM changelog c WHERE c.activity_agent_id IS NOT NULL AND (c.new_status IS DISTINCT FROM c.old_status OR c.new_notes IS DISTINCT FROM c.old_notes OR c.new_call_count IS DISTINCT FROM c.old_call_count)
      UNION ALL
      SELECT n.created_by AS worked_agent_id, n.lead_id FROM public.lead_quick_notes n WHERE n.created_at >= day_start AND n.created_at < day_end AND n.created_by IS NOT NULL
      UNION ALL
      SELECT (r.user_id)::uuid AS worked_agent_id, r.lead_id::uuid FROM public.lead_reminders r WHERE r.created_at >= day_start AND r.created_at < day_end AND r.user_id IS NOT NULL AND r.user_id ~* '^[0-9a-f-]{36}$' AND r.lead_id ~* '^[0-9a-f-]{36}$'
    ) worked_rows
    WHERE worked_rows.worked_agent_id IS NOT NULL
    GROUP BY worked_rows.worked_agent_id
  ),
  active_eod AS (
