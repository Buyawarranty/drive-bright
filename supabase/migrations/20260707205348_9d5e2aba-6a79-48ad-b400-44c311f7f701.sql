...
-- Admin setting: what to do after 6 failed no-answer attempts
ALTER TABLE public.shark_tank_settings
  ADD COLUMN IF NOT EXISTS no_answer_final_action text NOT NULL DEFAULT 'lost'
  CHECK (no_answer_final_action IN ('lost','new'));

-- Helper: next UK working day 9am, returned as timestamptz
CREATE OR REPLACE FUNCTION public.open_pool_next_working_day_9am()
RETURNS timestamptz
LANGUAGE plpgsql
IMMUTABLE
...
DECLARE
  ...
  _dow     integer;
BEGIN
  _uk_now := (now() AT TIME ZONE 'Europe/London');
  _target := date_trunc('day', _uk_now) + interval '1 day' + interval '9 hours';
  LOOP
    _dow := EXTRACT(ISODOW FROM _target); -- 1=Mon..7=Sun
    EXIT WHEN _dow < 6;
    _target := _target + interval '1 day';
  END LOOP;
  RETURN _target AT TIME ZONE 'Europe/London';
END;
$$;

GRANT EXECUTE ON FUNCTION public.open_pool_next_working_day_9am() TO authenticated, service_role;

-- Rewrite log_outcome with retry scheduling + give-up rule
...
BEGIN
  ...
  CASE _outcome
    ...
    WHEN 'no_answer' THEN
      ...
      IF _attempt_num >= 6 THEN
        -- Give up per admin setting
        SELECT no_answer_final_action INTO _final_action
          FROM shark_tank_settings WHERE id = 1;
        _final_action := COALESCE(_final_action, 'lost');
        _new_queue := 'nurture_queue';
        _new_owner := NULL;
    ...
    WHEN 'not_interested' THEN
      ...
      _lost_tag := CASE _reason
        ...
        ELSE NULL
      END;
      IF _lost_tag IS NOT NULL THEN
        _tags := ARRAY[_lost_tag];
      END IF;
    ELSE
  ...
  UPDATE sales_leads
     SET call_outcome   = _outcome,
         ...
         lost_reason    = CASE
                            ...
                            WHEN _outcome = 'no_answer' AND _attempt_num >= 6 AND _new_status = 'lost'
                              THEN 'Could not contact after 6 attempts'
                            ELSE lost_reason
                          END,
         next_action_at = CASE
