...
ALTER TABLE public.agent_distribution_caps
  DROP CONSTRAINT IF EXISTS agent_distribution_caps_priority_range;
ALTER TABLE public.agent_distribution_caps
  ADD CONSTRAINT agent_distribution_caps_priority_range
  CHECK (priority IS NULL OR priority BETWEEN 1 AND 4);

CREATE INDEX IF NOT EXISTS agent_distribution_caps_priority_idx
  ON public.agent_distribution_caps (priority);

COMMENT ON COLUMN public.agent_distribution_caps.priority IS
  'Assignment tier for New Leads. 1 = first pick, 4 = last. NULL = no priority (only picked if all numbered tiers are unavailable). Within the same tier, existing share%/round-robin logic applies.';

-- 2. Picker: priority-aware
...
BEGIN
  ...
  -- Find the LOWEST priority tier with at least one eligible agent.
  -- Priority NULL is treated as 999 (last resort).
  ...
  -- Overflow (unchanged — priority is a within-team preference)
