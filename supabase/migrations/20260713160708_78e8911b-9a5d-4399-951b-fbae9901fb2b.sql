...
-- Alternately assigns unassigned morning_call_queue leads to a
-- round-robin agent or to the Open Pool (self-serve via shark_tank_pool).
-- =====================================================================
...
DECLARE
  ...
  _skipped int := 0;
BEGIN
  -- Cron calls have auth.uid() = NULL; user calls must be management
  ...
  LOOP
    ...
    IF _next_slot = 'rr' THEN
      ...
      IF _agent IS NOT NULL THEN
        ...
        CONTINUE;
      END IF;
      -- No RR agent available → fall through to pool for this lead
    END IF;

    -- Pool slot (or RR spill)
    UPDATE public.sales_leads
    ...
    BEGIN
      INSERT INTO public.shark_tank_audit(lead_id, action, payload)
...
GRANT EXECUTE ON FUNCTION public.open_pool_drain_morning_queue(integer) TO authenticated, service_role;

-- =====================================================================
-- Schedule at 09:00 UK Mon–Sat. pg_cron runs UTC, so schedule at both
-- 08:00 and 09:00 UTC and gate on Europe/London local hour = 9 so we
-- fire exactly once whether UK is on GMT or BST.
-- =====================================================================
DO $$ BEGIN PERFORM cron.unschedule('open_pool_drain_morning_queue_utc09'); EXCEPTION WHEN OTHERS THEN NULL; END $$;
DO $$ BEGIN PERFORM cron.unschedule('open_pool_drain_morning_queue_utc08'); EXCEPTION WHEN OTHERS THEN NULL; END $$;
DO $$ BEGIN PERFORM cron.unschedule('open_pool_drain_morning_queue_gmt'); EXCEPTION WHEN OTHERS THEN NULL; END $$;
DO $$ BEGIN PERFORM cron.unschedule('open_pool_drain_morning_queue_bst'); EXCEPTION WHEN OTHERS THEN NULL; END $$;

SELECT cron.schedule(
  'open_pool_drain_morning_queue_utc09',
  '0 9 * * 1-6',
  $cron$
  DO $inner$
  BEGIN
    IF EXTRACT(HOUR FROM (now() AT TIME ZONE 'Europe/London')) = 9 THEN
      PERFORM public.open_pool_drain_morning_queue(500);
    END IF;
  END $inner$;
  $cron$
);
...
SELECT cron.schedule(
  'open_pool_drain_morning_queue_utc08',
  '0 8 * * 1-6',
  $cron$
  DO $inner$
  BEGIN
