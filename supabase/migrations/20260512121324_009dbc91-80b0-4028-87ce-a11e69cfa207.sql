...
CREATE OR REPLACE FUNCTION public.is_agent_on_duty(p_admin_user_id uuid)
RETURNS boolean
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  ...
  v_match boolean;
BEGIN
  v_now_uk := (now() AT TIME ZONE 'Europe/London');
  v_dow := EXTRACT(DOW FROM v_now_uk)::smallint;
  v_time := v_now_uk::time;

  SELECT EXISTS(SELECT 1 FROM agent_schedules WHERE admin_user_id = p_admin_user_id) INTO v_has_any;
  -- If the agent has no schedule rows at all, treat them as always on duty (back-compat)
  ...
  SELECT EXISTS(
    ...
    WHERE s.admin_user_id = p_admin_user_id
      ...
      AND v_time >= s.shift_start
      AND v_time < s.shift_end
      AND NOT (
        s.break_start IS NOT NULL
        AND s.break_end IS NOT NULL
        AND v_time >= s.break_start
        AND v_time < s.break_end
      )
  ) INTO v_match;
...
BEGIN
    ...
    -- Determine if anyone is currently on duty among eligible agents.
    -- If no one is on duty, fall back to ignoring duty filter (so leads still flow).
    SELECT EXISTS(
    ...
    IF v_settings.distribution_mode = 'percentage' THEN
        ...
        WHERE au.is_active = true
          ...
          AND (NOT v_any_on_duty OR public.is_agent_on_duty(adc.admin_user_id));
    ...
    -- ROUND ROBIN (strict sort_order, restricted to on-duty agents when available)
