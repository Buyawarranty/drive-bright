...
BEGIN
  ...
  -- Resolve caller's admin_users row (auth.uid() → admin_users.id)
  ...
  -- Permit when:
  --   * lead is locked to this agent (original pool reservation flow), OR
  --   * caller owns the lead (owner_agent matches _agent or caller's admin id), OR
  --   * lead is unowned/unlocked (any authenticated agent can claim + log), OR
