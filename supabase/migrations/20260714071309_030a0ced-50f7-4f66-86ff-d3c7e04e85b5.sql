...
-- Seed the new Sunday roster setting (default = current solo agent if set)
INSERT INTO public.lead_settings (setting_key, setting_value)
SELECT 'weekend_sunday_roster',
       COALESCE(
...
-- =====================================================================
-- auto_assign_lead_round_robin: Sunday roster with round-robin
-- Reads weekend_sunday_roster (jsonb array of uuid strings).
-- Falls back to legacy weekend_solo_agent_id if roster empty.
-- Picks the active roster agent with the OLDEST last_assigned_at,
-- ties broken by fewest Sunday assignments today.
-- =====================================================================
...
BEGIN
  ...
  IF v_uk_dow = 0 THEN
    -- Sunday: pick from configured roster (round-robin by last_assigned_at)
    BEGIN
      SELECT COALESCE(
               ARRAY(
    ...
    -- Choose the least-recently-assigned active roster agent
    ...
    IF v_sunday_pick IS NOT NULL THEN
      ...
      NEW.assigned_at := now();
      NEW.queue := 'live_new';
      -- Bump last_assigned_at so RR rotates on next lead (do not enforce cap)
      BEGIN
    ...
    -- No active roster agents → pool + coverage-down flag
  ...
  -- ================================================================
  -- Weekday: original logic (unchanged)
  -- ================================================================
