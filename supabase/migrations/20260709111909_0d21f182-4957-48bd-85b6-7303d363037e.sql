...
DECLARE
  ...
  _pending int;
  _claimed int := 0;
  _cooldown interval := interval '7 days';
BEGIN
  ...
  WITH picked AS (
    ...
    WHERE sl.assigned_to IS NULL
      ...
      -- Cooldown: skip anything that was assigned to (i.e. worked by) another
      -- agent in the last 7 days, so two agents can't rapidly re-claim the
      -- same recontact leads.
      AND (sl.assigned_at IS NULL OR sl.assigned_at < now() - _cooldown)
      -- Also skip anything with a recent activity/note touch by another agent.
      AND NOT EXISTS (
        SELECT 1 FROM public.lead_activities la
        WHERE la.lead_id = sl.id
          AND la.created_at > now() - _cooldown
      )
      AND NOT EXISTS (
        SELECT 1 FROM public.lead_quick_notes lqn
        WHERE lqn.lead_id = sl.id
          AND lqn.created_at > now() - _cooldown
      )
