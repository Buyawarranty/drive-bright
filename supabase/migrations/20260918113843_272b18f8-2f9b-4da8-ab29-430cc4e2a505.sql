...
AS $function$
DECLARE _existing uuid; _picked uuid; _admin_id uuid; _auth_id uuid;
BEGIN
  SELECT id,user_id INTO _admin_id,_auth_id FROM public.admin_users WHERE (id=_agent OR user_id=_agent) AND is_active=true LIMIT 1;
  IF _admin_id IS NULL THEN RAISE EXCEPTION 'Active agent not found'; END IF;

  SELECT id INTO _existing FROM public.sales_leads
  ...
  WITH ranked_candidates AS MATERIALIZED (
    SELECT sl.id,sl.email,sl.vehicle_reg,
      CASE WHEN 'paid_google'=ANY(sl.auto_tags) THEN 1 WHEN 'paid_facebook'=ANY(sl.auto_tags) THEN 2
        ...
        WHEN 'website_quote'=ANY(sl.auto_tags) THEN 7 WHEN COALESCE(sl.call_count,0)=0 THEN 8
        WHEN sl.queue='nurture_queue' THEN 9 ELSE 10 END priority_band,
    ...
    WHERE sl.queue IN ('live_open_pool','morning_call_queue','retry_queue','callback_queue','nurture_queue')
      ...
      AND sl.owner_agent IS NULL AND sl.assigned_to IS NULL
    ...
    WHERE NOT EXISTS (SELECT 1 FROM public.customers c WHERE COALESCE(c.is_deleted,false)=false
      AND lower(COALESCE(c.status,'')) NOT IN ('cancelled','refunded')
      AND ((COALESCE(rc.email,'')<>'' AND lower(c.email)=lower(rc.email)) OR
        (COALESCE(rc.vehicle_reg,'')<>'' AND upper(regexp_replace(c.registration_plate,'\s+','','g'))=upper(regexp_replace(rc.vehicle_reg,'\s+','','g')))))
  )
  ...
  UPDATE public.sales_leads SET assigned_to=_admin_id, owner_agent=NULL, assigned_at=now(), pool_status='calling_locked',
    locked_by=COALESCE(_auth_id,_agent), locked_at=now(), last_action_at=now(), orr_first_call_deadline=now()+interval '120 seconds', updated_at=now()
  WHERE id=_picked;
