...
-- Drop old overloads
DROP FUNCTION IF EXISTS public.claim_recontact_leads_batch(integer);
...
BEGIN
  ...
  -- Pending gate: leads assigned to me that STILL have no note and no call log
  -- since they were assigned. Once every lead has at least one note or call, they
  -- can claim the next batch (no other restrictions).
  ...
  -- Eligibility: unassigned, non-terminal, older than 30 days.
  -- No assigned_at cooldown. Oldest first.
  WITH picked AS (
    SELECT sl.id
    FROM public.sales_leads sl
