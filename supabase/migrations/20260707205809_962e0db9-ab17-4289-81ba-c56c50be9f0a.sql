...
-- 1) Add high_priority tag on payment_link_sent (patch outcome function)
    ...
    WHEN 'no_answer' THEN
      ...
      ELSE
        ...
        _retry_at := CASE _attempt_num
          WHEN 1 THEN now() + interval '20 minutes'
...
-- 2) Mark payment received (called by agent/webhook)
CREATE OR REPLACE FUNCTION public.open_pool_mark_paid(_lead_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
GRANT EXECUTE ON FUNCTION public.open_pool_mark_paid(uuid) TO authenticated, service_role;

-- 3) Unpaid payment-link follow-up sweeper
CREATE OR REPLACE FUNCTION public.open_pool_check_unpaid_links()
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  ...
  _since interval;
BEGIN
  -- Reminder: 30 min unpaid, no reminder tag yet
  FOR _rec IN
    SELECT id, owner_agent, phone, first_name, last_name, last_action_at
      FROM sales_leads
     WHERE pool_status = 'payment_link_sent'
       AND COALESCE(is_paid, false) = false
       AND last_action_at IS NOT NULL
       AND last_action_at < now() - interval '30 minutes'
       AND NOT ('payment_link_reminder' = ANY(COALESCE(auto_tags, ARRAY[]::text[])))
  LOOP
    UPDATE sales_leads
       SET auto_tags = (
             SELECT ARRAY(SELECT DISTINCT unnest(
               COALESCE(auto_tags, ARRAY[]::text[]) || ARRAY['payment_link_reminder']
             ))
           )
     WHERE id = _rec.id;

    INSERT INTO public.system_event_logs (event_type, event_source, event_data)
    VALUES ('open_pool_payment_link_reminder', 'open_pool',
      jsonb_build_object(
        'lead_id', _rec.id, 'owner_agent', _rec.owner_agent,
        'name', trim(coalesce(_rec.first_name,'') || ' ' || coalesce(_rec.last_name,'')),
        'phone', _rec.phone, 'sent_at', _rec.last_action_at
      ));
    _count := _count + 1;
  END LOOP;

  -- Manager alert: 24h unpaid and no newer agent action since the link was sent
  FOR _rec IN
    SELECT id, owner_agent, phone, first_name, last_name, last_action_at
      FROM sales_leads
     WHERE pool_status = 'payment_link_sent'
       ...
       AND NOT ('payment_link_manager_alert' = ANY(COALESCE(auto_tags, ARRAY[]::text[])))
  LOOP
    UPDATE sales_leads
       SET auto_tags = (
             SELECT ARRAY(SELECT DISTINCT unnest(
               COALESCE(auto_tags, ARRAY[]::text[]) || ARRAY['payment_link_manager_alert']
             ))
           )
     WHERE id = _rec.id;

    INSERT INTO public.system_event_logs (event_type, event_source, event_data)
    VALUES ('open_pool_payment_link_stalled', 'open_pool',
      jsonb_build_object(
        'lead_id', _rec.id, 'owner_agent', _rec.owner_agent,
        'name', trim(coalesce(_rec.first_name,'') || ' ' || coalesce(_rec.last_name,'')),
        'phone', _rec.phone, 'sent_at', _rec.last_action_at,
        'unpaid_hours', EXTRACT(EPOCH FROM (now() - _rec.last_action_at))/3600
      ));
    _count := _count + 1;
  END LOOP;
...
GRANT EXECUTE ON FUNCTION public.open_pool_check_unpaid_links() TO authenticated, service_role;

DO $$
DECLARE _jid bigint;
BEGIN
  FOR _jid IN SELECT jobid FROM cron.job WHERE jobname = 'open-pool-check-unpaid-links'
  LOOP PERFORM cron.unschedule(_jid); END LOOP;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

SELECT cron.schedule(
  'open-pool-check-unpaid-links',
  '*/5 * * * *',
  $$SELECT public.open_pool_check_unpaid_links();$$
);
