...
BEGIN
  FOR v_cart IN
    ...
    WHERE ac.is_converted = false
      AND NOT EXISTS (SELECT 1 FROM sales_leads sl WHERE sl.abandoned_cart_id = ac.id)
  ...
  LOOP
    ...
    BEGIN
      ...
      INSERT INTO sales_leads (
        ...
        created_at, updated_at, last_activity_date, assigned_to, assigned_at
      ) VALUES (
        ...
        NULLIF(btrim(v_cart.vehicle_type), ''), NULLIF(btrim(v_cart.mileage), ''),
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      VALUES ('orphan_migration_failed', 'migrate_orphan_carts_to_leads',
        jsonb_build_object('cart_id', v_cart.id, 'email', v_cart.email), SQLERRM);
    END;
  END LOOP;
...
CREATE INDEX IF NOT EXISTS idx_sales_leads_email_lower ON public.sales_leads (lower(btrim(email)));
CREATE INDEX IF NOT EXISTS idx_sales_leads_phone_digits ON public.sales_leads ((regexp_replace(COALESCE(phone, ''), '[^0-9]', '', 'g')));
CREATE INDEX IF NOT EXISTS idx_abandoned_carts_email_lower ON public.abandoned_carts (lower(btrim(email)));