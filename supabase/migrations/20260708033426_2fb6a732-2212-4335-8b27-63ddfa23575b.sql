...
BEGIN
  ...
  -- Solo mode short-circuit (respects whatever mode the solo agent is in)
  ...
  -- Try Open Pool agents first, then Round Robin agents
  FOR v_i IN 1..array_length(v_modes, 1) LOOP
    ...
    -- No eligible agents in this mode — skip to next mode
    IF NOT v_any_on_duty THEN
      CONTINUE;
    END IF;

    -- Percentage distribution (per mode)
    ...
    -- Round-robin rotation within this mode
  ...
  -- Final fallback: overflow recipients (team-scoped calls just return NULL)
  IF p_team_id IS NOT NULL THEN
    RETURN NULL;
  END IF;
