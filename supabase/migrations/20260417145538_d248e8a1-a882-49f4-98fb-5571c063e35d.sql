...
BEGIN
  ...
  IF v_derived_source = 'website' THEN
    IF EXISTS (SELECT 1 FROM public.abandoned_carts
      WHERE lower(btrim(email)) = v_clean_email AND cart_metadata IS NOT NULL
        AND (cart_metadata::text LIKE '%gclid%') LIMIT 1) THEN
      v_derived_source := 'google_ad';
    ELSIF EXISTS (SELECT 1 FROM public.abandoned_carts
      WHERE lower(btrim(email)) = v_clean_email AND cart_metadata IS NOT NULL
        AND (cart_metadata::text LIKE '%fbclid%' OR cart_metadata::text LIKE '%fb_referrer%'
          OR cart_metadata->>'utm_source' IN ('facebook','fb','ig','instagram')) LIMIT 1) THEN
    ...
    IF v_terminal_lead_id IS NOT NULL THEN
      ...
      VALUES ('lead_blocked_by_terminal', 'auto_create_lead_trigger',
        jsonb_build_object('cart_id', NEW.id, 'email', v_clean_email, 'phone', NEW.phone,
          'blocked_by_lead_id', v_terminal_lead_id, 'match_type', 'phone',
          'lead_source', v_derived_source::text, 'cart_metadata', NEW.cart_metadata,
          'paid_bypass', (v_derived_source IN ('google_ad','social_ad'))::text));
      IF v_derived_source NOT IN ('google_ad','social_ad') THEN RETURN NEW; END IF;
    END IF;
  END IF;

  SELECT id INTO v_terminal_lead_id FROM public.sales_leads
  WHERE lower(btrim(email)) = v_clean_email AND status IN ('converted','lost','fake_lead') LIMIT 1;
  IF v_terminal_lead_id IS NOT NULL THEN
    INSERT INTO system_event_logs (event_type, event_source, event_data)
    VALUES ('lead_blocked_by_terminal', 'auto_create_lead_trigger',
      jsonb_build_object('cart_id', NEW.id, 'email', v_clean_email, 'phone', NEW.phone,
        'blocked_by_lead_id', v_terminal_lead_id, 'match_type', 'email',
        'lead_source', v_derived_source::text, 'cart_metadata', NEW.cart_metadata,
        'paid_bypass', (v_derived_source IN ('google_ad','social_ad'))::text));
    IF v_derived_source NOT IN ('google_ad','social_ad') THEN RETURN NEW; END IF;
  END IF;
  ...
  BEGIN
    ...
    IF v_assigned_agent IS NOT NULL THEN
      ...
      SELECT user_id INTO v_assigned_auth_uid FROM public.admin_users WHERE id = v_assigned_agent;
      IF v_assigned_auth_uid IS NOT NULL THEN
        UPDATE public.abandoned_carts SET contacted_by = v_assigned_auth_uid WHERE id = NEW.id;
      END IF;
    END IF;
  EXCEPTION WHEN OTHERS THEN
    ...
    VALUES ('lead_creation_failed', 'auto_create_lead_trigger',
      jsonb_build_object('cart_id', NEW.id, 'email', v_clean_email, 'phone', NEW.phone), SQLERRM);
  END;
