...
-- Fix: agents on assignment_mode='open_pool' should NEVER be auto-assigned.
-- They only receive leads by clicking "Take Next Lead" in the Open Lead Pool.

-- 1) pick_agent_for_distribution: only iterate round_robin mode; skip open_pool
--    agents in overflow too.
...
BEGIN
  ...
  -- Solo mode: still respect assignment_mode (solo agent on open_pool = no auto-assign)
  ...
  -- Only round_robin agents are eligible for auto-assignment.
  SELECT EXISTS(
    SELECT 1
    FROM public.agent_distribution_caps adc
    JOIN public.admin_users au ON au.id = adc.admin_user_id
  ...
  -- Overflow: also skip open_pool agents so overflow can't bypass their preference.
...
-- 2) auto_assign_lead_round_robin: if a preassigned agent is on open_pool mode,
--    strip the assignment so the lead goes into the Open Pool instead.
