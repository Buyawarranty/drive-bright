...
BEGIN
  ...
  -- STRICT PER-VISIT ATTRIBUTION:
  -- lead_source is derived ONLY from the current cart's metadata.
  -- We deliberately do NOT inherit attribution from past carts of the same email,
  -- because that caused all repeat organic visitors to be permanently mis-tagged
  -- as paid (google_ad / social_ad) once they had ever clicked an ad in the past.
  ...
  -- TERMINAL GUARD: only block ORGANIC repeat leads; paid sources always get a new lead
  IF length(v_clean_phone) >= 10 THEN
