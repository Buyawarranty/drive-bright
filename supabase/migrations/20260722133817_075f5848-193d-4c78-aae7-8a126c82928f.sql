...
-- 1) Resync today's counters to match reality (fixes drift that put Freddie & sales@ at 23 vs real 5/4)
UPDATE public.agent_distribution_caps adc
SET assigned_today = COALESCE(sub.cnt, 0),
    cap_reset_date = (now() AT TIME ZONE 'Europe/London')::date
FROM (
  SELECT au.id AS admin_user_id,
         (SELECT COUNT(*) FROM public.sales_leads sl
           WHERE sl.assigned_to = au.id
             AND sl.assigned_at >= ((now() AT TIME ZONE 'Europe/London')::date) AT TIME ZONE 'Europe/London'
         ) AS cnt
  FROM public.admin_users au
) sub
WHERE adc.admin_user_id = sub.admin_user_id;

-- 2) Patch pick_agent_for_distribution to self-heal counter drift on every call
...
DECLARE
  ...
  v_min_priority int;
BEGIN
  -- Self-heal: reconcile every agent's assigned_today with reality before selection.
  -- Prevents phantom "over cap" errors when the stored counter drifts above the true
  -- number of leads actually assigned to the agent today (London time).
  UPDATE public.agent_distribution_caps adc
