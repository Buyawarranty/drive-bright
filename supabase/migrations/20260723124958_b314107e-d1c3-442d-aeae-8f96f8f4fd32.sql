...
BEGIN
  ...
  v_can_override := v_caller_admin.role IN ('super_admin','admin','sales_manager','performance_manager','sales_lead');

  -- CRITICAL: bulk manager reassignment must bypass trg_protect_touched_leads,
  -- otherwise any lead with a note / call log / call_count>0 silently snaps back
  -- to its previous owner. Only managers (and sales_lead offboarding) get this.
