...
BEGIN
  ...
  UPDATE public.sales_leads SET assigned_to=_admin_id,pool_status='calling_locked',locked_by=_admin_id,locked_at=now(),last_action_at=now(),orr_first_call_deadline=now()+interval '120 seconds',updated_at=now() WHERE id=_picked;
