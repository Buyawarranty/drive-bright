...
-- ============================================================
-- FIX 1: Rebuild orr_queue_dashboard_snapshot with correct columns
-- ============================================================
...
BEGIN
  ...
  agents AS (
    -- ORR agents only, joined via admin_users.id (adc.admin_user_id references admin_users.id)
    ...
    JOIN public.agent_distribution_caps adc
      ON adc.admin_user_id = au.id AND adc.assignment_mode = 'open_pool'
...
-- ============================================================
-- FIX 2: ORR handoff trigger — only for agents whose distribution mode is open_pool
-- Fires AFTER assignment; classifies live vs overnight and starts the 2-min timer.
-- RR agents are completely untouched.
-- ============================================================
CREATE OR REPLACE FUNCTION public.orr_handoff_on_assign()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  ...
  v_kind text;
BEGIN
  -- Only act when assignment newly appears
  IF NEW.assigned_to IS NULL THEN RETURN NEW; END IF;
  IF TG_OP = 'UPDATE' AND OLD.assigned_to IS NOT DISTINCT FROM NEW.assigned_to THEN
    RETURN NEW;
  END IF;

  -- Skip if lead already has an ORR deadline (avoid double-trigger)
  IF NEW.orr_first_call_deadline IS NOT NULL AND NEW.orr_first_call_deadline > now() THEN
    RETURN NEW;
  END IF;

  -- Only handoff if assigned agent is in open_pool mode
  SELECT adc.assignment_mode INTO v_mode
  FROM public.agent_distribution_caps adc
  WHERE adc.admin_user_id = NEW.assigned_to
  LIMIT 1;

  IF v_mode IS DISTINCT FROM 'open_pool' THEN
    RETURN NEW;  -- RR agent, do nothing
  END IF;

  -- Skip closed statuses
  IF COALESCE(NEW.status::text,'') IN ('converted','lost','fake_lead','not_interested','do_not_contact','wrong_number','dormant','archived') THEN
    RETURN NEW;
  END IF;

  -- Classify intake to decide live vs overnight
  SELECT * INTO v_class FROM public.orr_classify_intake(COALESCE(NEW.created_at, now()));
  v_kind := COALESCE(v_class.intake_class, 'overnight');

  -- Call the existing ORR attempt-one flow (best effort; swallow errors)
  BEGIN
    PERFORM public.orr_assign_attempt_one(NEW.id, NEW.assigned_to, v_kind);
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'orr_handoff_on_assign: attempt_one failed for lead % agent %: %',
      NEW.id, NEW.assigned_to, SQLERRM;
  END;
...
DROP TRIGGER IF EXISTS trg_orr_handoff_on_assign ON public.sales_leads;
CREATE TRIGGER trg_orr_handoff_on_assign
AFTER INSERT OR UPDATE OF assigned_to ON public.sales_leads
FOR EACH ROW EXECUTE FUNCTION public.orr_handoff_on_assign();
