...
BEGIN
  ...
  agents AS (
    SELECT au.id AS agent_id,
           ...
           (SELECT COUNT(*) FROM public.lead_reminders lr
              WHERE lr.user_id = au.id::text
                AND lr.status = 'pending'
                AND (COALESCE(lr.snoozed_until, lr.reminder_time) AT TIME ZONE 'Europe/London')::date = v_today_london) AS callbacks_owned,
           (SELECT COUNT(*) FROM public.sales_leads sl
              WHERE sl.assigned_to = au.id
  ...
  SELECT jsonb_build_object(
    ...
    'agents', COALESCE((
      ...
      ) ORDER BY agent_name) FROM agents), '[]'::jsonb)
  ) INTO v_result;
  RETURN v_result;
END;
$function$;