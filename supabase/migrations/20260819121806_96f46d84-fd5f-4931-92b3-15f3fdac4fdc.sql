...
BEGIN
  ...
  v_call_at := coalesce(NEW.started_at, NEW.created_at, now());

  -- Skip entirely when an agent already dialled this number via Zoiper / Dial 9
  -- within 30 minutes of the missed call (before or after).
  IF v_is_missed AND length(v_tail9) = 9 THEN
    SELECT EXISTS (
      SELECT 1 FROM public.phone_events pe
      WHERE pe.phone_number IS NOT NULL
        AND right(regexp_replace(pe.phone_number, '\D', '', 'g'), 9) = v_tail9
        AND pe.created_at BETWEEN v_call_at - interval '30 minutes' AND v_call_at + interval '30 minutes'
    ) INTO v_agent_called;

    IF v_agent_called THEN
      RETURN NEW;
    END IF;
  END IF;
  ...
  IF v_lead_id IS NULL THEN
    ...
    VALUES (
      ...
      'Inbound CallRail call (' || v_status || ')' ||
        ...
        ' at ' || to_char(v_call_at, 'DD Mon YYYY HH24:MI') ||
