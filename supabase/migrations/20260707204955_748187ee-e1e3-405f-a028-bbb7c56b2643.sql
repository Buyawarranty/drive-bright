...
-- Auto-tag trigger on lead creation (source + timing)
...
BEGIN
  ...
  -- Overnight tag: arrived after 18:00 UK time (or before 6am)
  _hour_uk := EXTRACT(HOUR FROM (COALESCE(NEW.created_at, now()) AT TIME ZONE 'Europe/London'));
  IF _hour_uk >= 18 OR _hour_uk < 6 THEN
...
DROP TRIGGER IF EXISTS trg_open_pool_auto_tag ON public.sales_leads;
CREATE TRIGGER trg_open_pool_auto_tag
  BEFORE INSERT ON public.sales_leads
  FOR EACH ROW EXECUTE FUNCTION public.open_pool_auto_tag_on_insert();

-- Extend outcome function with lost-reason and invalid tags
