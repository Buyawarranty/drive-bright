CREATE OR REPLACE FUNCTION public.resolve_sale_credit(
  _sale_credit uuid,
  _payment_confirmed_by uuid,
  _quote_sent_by uuid,
  _assigned_to uuid
)
RETURNS uuid
...
AS $$
  SELECT COALESCE(
    ...
    (
      SELECT cand.id
      FROM (
        VALUES (1, _payment_confirmed_by), (2, _quote_sent_by), (3, _assigned_to)
      ) AS v(ord, id)
      JOIN public.admin_users cand ON cand.id = v.id
      WHERE cand.role IN ('sales', 'sales_lead')
      ORDER BY v.ord
      LIMIT 1
    )
  );
...
GRANT EXECUTE ON FUNCTION public.resolve_sale_credit(uuid, uuid, uuid, uuid) TO authenticated, service_role;
