...
BEGIN
  ...
  -- Resolve admin_users.id for the acting agent (assigned_to stores admin_users.id)
  ...
  UPDATE sales_leads
     SET call_outcome   = _outcome,
         ...
         owner_agent    = CASE
                          ...
                          END,
         -- Sync the agent-facing "My leads" ownership so a lead the agent has
         -- taken (Spoken to / Follow-up / Quote sent / Policy sent / Payment
         -- link sent / Sold) shows up in their own Leads list.
         assigned_to    = CASE
                            WHEN _keep_lock AND _agent_admin_id IS NOT NULL THEN _agent_admin_id
                            ELSE assigned_to
                          END,
         assigned_at    = CASE
                            WHEN _keep_lock AND _agent_admin_id IS NOT NULL
