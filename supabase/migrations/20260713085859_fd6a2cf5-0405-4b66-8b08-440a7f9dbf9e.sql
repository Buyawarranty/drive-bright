-- Speed up Open Lead Pool "Take next lead" by ranking a small candidate set
-- before doing the heavier active-customer duplicate checks.
CREATE INDEX IF NOT EXISTS idx_sales_leads_open_pool_picker
  ON public.sales_leads (created_at DESC, next_action_at)
  WHERE queue IN ('live_open_pool','morning_call_queue','retry_queue','callback_queue','nurture_queue')
    AND COALESCE(is_paid, false) = false
    AND status NOT IN ('converted'::lead_status, 'fake_lead'::lead_status, 'lost'::lead_status)
    AND (pool_status IS NULL OR pool_status IN ('new','callback_booked','contacted'));

CREATE INDEX IF NOT EXISTS idx_customers_active_email_norm
  ON public.customers (lower(trim(email)))
  WHERE COALESCE(is_deleted, false) = false
    AND lower(COALESCE(status, '')) NOT IN ('cancelled','refunded')
    AND COALESCE(email, '') <> '';

CREATE INDEX IF NOT EXISTS idx_customers_active_reg_norm
  ON public.customers (upper(regexp_replace(registration_plate, '\s+', '', 'g')))
  WHERE COALESCE(is_deleted, false) = false
    AND lower(COALESCE(status, '')) NOT IN ('cancelled','refunded')
    AND COALESCE(registration_plate, '') <> '';
...
DECLARE
  ...
  _picked uuid;
BEGIN
  -- One active lock per agent. If the agent refreshes/retries while a lock is
  -- active, return that same lead instead of searching the whole pool.
