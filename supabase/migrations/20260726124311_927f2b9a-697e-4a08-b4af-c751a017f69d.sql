...
AS $function$
  ...
  WHERE regexp_replace(upper(coalesce(vehicle_reg, '')), '[^A-Z0-9]', '', 'g') = regexp_replace(upper(coalesce(_vehicle_reg, '')), '[^A-Z0-9]', '', 'g')
...
ALTER TABLE public.sales_leads
  DROP CONSTRAINT IF EXISTS sales_leads_queue_check;

ALTER TABLE public.sales_leads
  ADD CONSTRAINT sales_leads_queue_check
  CHECK (
    queue IS NULL OR queue = ANY (ARRAY[
      'live_new'::text,
      'live_open_pool'::text,
      'overnight_queue'::text,
      'morning_call_queue'::text,
      'retry_queue'::text,
      'callback_queue'::text,
      'owned_by_agent'::text,
      'nurture_queue'::text,
...
BEGIN
  ...
  v_clean_reg := regexp_replace(upper(COALESCE(NEW.vehicle_reg, '')), '[^A-Z0-9]', '', 'g');
  ...
  -- Do not resurrect a terminal lead when it is clearly the same customer.
  -- A phone number alone is not enough: test numbers and reused numbers were suppressing genuine new leads.
  ...
  IF length(v_clean_phone) >= 10 AND length(v_clean_reg) > 0 THEN
    SELECT id INTO v_terminal_lead_id FROM public.sales_leads
    WHERE regexp_replace(COALESCE(phone, ''), '[^0-9]', '', 'g') = v_clean_phone
      AND regexp_replace(upper(COALESCE(vehicle_reg, '')), '[^A-Z0-9]', '', 'g') = v_clean_reg
      AND status IN ('converted','lost','fake_lead')
    LIMIT 1;
    IF v_terminal_lead_id IS NOT NULL THEN
      RETURN NEW;
  ...
  WHERE lower(btrim(email)) = v_clean_email
    AND status NOT IN ('converted','lost','fake_lead')
    AND (v_clean_reg = '' OR regexp_replace(upper(COALESCE(vehicle_reg, '')), '[^A-Z0-9]', '', 'g') = v_clean_reg)
    AND (created_at > now() - interval '7 days' OR assigned_to IS NOT NULL OR call_count > 0 OR notes IS NOT NULL)
  ORDER BY CASE WHEN assigned_to IS NOT NULL THEN 0 ELSE 1 END, created_at DESC LIMIT 1;
...
SET updated_at = now()
WHERE id = '3ddbd0b6-af85-4e99-a794-7b2e06f95337';