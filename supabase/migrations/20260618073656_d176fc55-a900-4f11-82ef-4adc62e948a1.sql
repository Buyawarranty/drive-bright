-- 1. Extend can_manage_lead_routing to include sales_manager
...
AS $function$
  SELECT EXISTS (
    ...
    WHERE user_id = _user_id
      AND role IN ('super_admin','admin','performance_manager','sales_manager')
      AND is_active = true
  );
$function$;

-- 2. Helper: pick an agent for either a team (p_team_id NOT NULL) or global (NULL).
-- When p_team_id IS NULL, this function reproduces the legacy trigger behaviour byte-for-byte.
...
BEGIN
  -- Pick settings: team override else global
  ...
  -- Round robin (team-scoped state row)
  ...
  -- Team path returns NULL so trigger can fall through to global; only global path drains to overflow.
  IF p_team_id IS NOT NULL THEN
    RETURN NULL;
  END IF;

  -- OVERFLOW (global only)
...
-- 3. Rewrite the trigger to try team routing first, then fall back to the existing global logic.
...
BEGIN
  ...
  -- Try team-routed assignment for the lead's source (highest priority first)
  FOR v_team_rule IN
    SELECT r.team_id
  ...
  -- Fallback: existing global behaviour
...
-- 4. Grant Sales Managers access to distribution config tables (additive — does not weaken existing policies)
CREATE POLICY "Lead routing managers can view distribution settings"
  ON public.lead_distribution_settings FOR SELECT
  USING (public.can_manage_lead_routing(auth.uid()));

CREATE POLICY "Lead routing managers can insert distribution settings"
  ON public.lead_distribution_settings FOR INSERT
  WITH CHECK (public.can_manage_lead_routing(auth.uid()));

CREATE POLICY "Lead routing managers can update distribution settings"
  ON public.lead_distribution_settings FOR UPDATE
  USING (public.can_manage_lead_routing(auth.uid()))
  WITH CHECK (public.can_manage_lead_routing(auth.uid()));

CREATE POLICY "Lead routing managers can delete distribution settings"
  ON public.lead_distribution_settings FOR DELETE
  USING (public.can_manage_lead_routing(auth.uid()));

CREATE POLICY "Lead routing managers can manage round robin state"
  ON public.round_robin_state FOR ALL
  USING (public.can_manage_lead_routing(auth.uid()))
  WITH CHECK (public.can_manage_lead_routing(auth.uid()));

CREATE POLICY "Lead routing managers can manage overflow rr state"
  ON public.overflow_round_robin_state FOR ALL
  USING (public.can_manage_lead_routing(auth.uid()))
  WITH CHECK (public.can_manage_lead_routing(auth.uid()));

CREATE POLICY "Lead routing managers can manage overflow recipients"
  ON public.overflow_recipients FOR ALL
  USING (public.can_manage_lead_routing(auth.uid()))
  WITH CHECK (public.can_manage_lead_routing(auth.uid()));

CREATE POLICY "Lead routing managers can manage agent caps"
