ALTER TABLE public.customers ADD COLUMN IF NOT EXISTS is_direct_website_sale boolean DEFAULT false;
...
BEGIN
  ...
  WITH attributed AS (
    SELECT
      CASE
        WHEN c.is_direct_website_sale THEN NULL
        ELSE public.resolve_sale_credit(c.sale_credit_admin_user_id, c.payment_confirmed_by, c.quote_sent_by, c.assigned_to)
      END AS aid,
      c.is_direct_website_sale AS direct,
    ...
    UNION ALL
    SELECT cc.agent_id, false, COALESCE(cc.deal_value, 0)::numeric
  ...
  classified AS (
    SELECT
      CASE
        WHEN a.direct THEN 'direct_website'
      ...
      END AS bucket,
      CASE
        WHEN a.direct THEN 'Direct website sale'
...
RETURNS TABLE(customer_id uuid, customer_name text, registration_plate text, amount numeric, signup_date timestamp with time zone, bucket text, current_label text, current_admin_user_id uuid)
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  ...
  LEFT JOIN public.admin_users au ON au.id = a.aid AND au.is_active
  LEFT JOIN public.lead_team_members tm ON tm.admin_user_id = au.id
  WHERE (a.direct = true OR a.aid IS NULL OR au.id IS NULL OR au.role NOT IN ('sales', 'sales_lead') OR tm.admin_user_id IS NULL)
  ORDER BY a.amount DESC, a.signup_date DESC;
END;
$function$;

CREATE OR REPLACE FUNCTION public.set_sale_credit_agent(p_customer_id uuid, p_admin_user_id uuid, p_direct_website_sale boolean DEFAULT false)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  IF p_direct_website_sale THEN
    UPDATE public.customers
    SET sale_credit_admin_user_id = NULL,
        is_direct_website_sale = true,
        sale_credit_overridden_by = auth.uid(),
        sale_credit_overridden_at = now(),
        sale_credit_override_reason = 'Direct website sale',
        updated_at = now()
  ...
  SET sale_credit_admin_user_id = p_admin_user_id,
      is_direct_website_sale = false,
      sale_credit_overridden_by = auth.uid(),
      sale_credit_overridden_at = now(),
      sale_credit_override_reason = COALESCE(sale_credit_override_reason, 'Manager assigned via scoreboard'),
      updated_at = now()
  WHERE id = p_customer_id;
END;
$function$;

GRANT EXECUTE ON FUNCTION public.get_scoreboard_reconciliation(timestamp with time zone, timestamp with time zone) TO authenticated, service_role;
...
GRANT EXECUTE ON FUNCTION public.set_sale_credit_agent(uuid, uuid, boolean) TO authenticated, service_role;