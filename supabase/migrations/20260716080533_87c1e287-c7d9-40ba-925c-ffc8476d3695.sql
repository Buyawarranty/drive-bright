  ...
  upd AS (
    UPDATE sales_leads sl
       SET owner_agent          = _target_admin_id,
           ...
           last_action_at       = now(),
           -- Bump the "activity" date so re-surfaced older leads appear in
           -- the receiving agent's current date-filtered live view. Without
           -- this, leads created days ago stay invisible under a "Today"
           -- date chip even though they're freshly assigned.
