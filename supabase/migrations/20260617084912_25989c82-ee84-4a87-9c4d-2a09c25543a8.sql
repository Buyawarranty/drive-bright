...
BEGIN
  ...
  v_ab_variant := lower(COALESCE(NEW.cart_metadata->>'ab_variant', ''));
  ...
  -- VARIANT B GUARD: do not create a brand-new sales lead when the user
  -- submitted step 2b without a phone number. Abandoned cart + email
  -- marketing still capture them; they just don't pollute New Leads
  -- because agents have nothing to call.
  IF v_ab_variant = 'b' AND length(v_clean_phone) = 0 THEN
    RETURN NEW;
  END IF;
