...
BEGIN
  ...
  -- ── Owner-sticky: a customer already being worked stays with the same agent ──
  IF NEW.assigned_to IS NULL THEN
    BEGIN
      ...
      IF v_phone_tail IS NOT NULL OR v_email IS NOT NULL THEN
        SELECT sl.id, sl.assigned_to
          INTO v_sticky_lead, v_sticky_owner
    ...
    IF v_sticky_owner IS NOT NULL THEN
      ...
      NEW.queue := 'live_new';
      NEW.auto_tags := (
        SELECT ARRAY(SELECT DISTINCT unnest(COALESCE(NEW.auto_tags, ARRAY[]::text[]) || ARRAY['same_customer_sticky']))
      );
      BEGIN
      ...
      BEGIN
        ...
        VALUES (NEW.id, v_sticky_owner, NULL, 'same_customer_sticky',
                format('Repeat customer already worked on lead %s → kept with existing owner (no round-robin)', v_sticky_lead));
      EXCEPTION WHEN OTHERS THEN NULL; END;
      RETURN NEW;
    END IF;
  END IF;

  -- NOTE: weekend-specific routing (Saturday open pool / Sunday roster) removed.
  -- Every day now uses the same standard round robin over agents that are switched on.
