...
/** Sends an agent's reply out through WATI and stores it against the conversation. */
Deno.serve(async (req) => {
  ...
  const url = `${endpoint}/api/v1/sendSessionMessage/${conv.phone_normalized}?messageText=${encodeURIComponent(message)}`;
  ...
  if (!watiRes.ok || watiBody?.ok === false || watiBody?.result === false) {
    console.error(`WATI send failed [${watiRes.status}]: ${watiText}`);
    return json({ error: 'wati_send_failed', status: watiRes.status, details: watiBody }, watiRes.status === 200 ? 502 : watiRes.status);
  }

  const now = new Date().toISOString();
  const watiMessageId =
    watiBody?.message?.whatsappMessageId || watiBody?.message?.id || `out-${conversationId}-${Date.now()}`;
