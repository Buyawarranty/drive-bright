...
export const ManualVehicleEntryCard: React.FC<ManualVehicleEntryCardProps> = ({
  ...
  const canSubmit =
    !!make.trim() && miles >= 100 && miles <= MAX_COVERED_MILEAGE && !isSubmitting;
    ...
    <div className="space-y-3 rounded-xl border-2 border-[#F0A500] bg-[#FFF8E5] p-4 text-left animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-2">
        <input
          ...
          className="h-12 rounded-lg border-2 border-[#E0B24A] bg-white px-3 text-base text-gray-900 outline-none focus:border-[#F0A500]"
        />
        <input
          id={`${idPrefix}-model`}
          value={model}
      ...
      <input
        ...
        className="w-full h-12 rounded-lg border-2 border-[#E0B24A] bg-white px-3 text-base text-gray-900 outline-none focus:border-[#F0A500]"
      />
      <MileageField
