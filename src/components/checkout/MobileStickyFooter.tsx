...
import { Lock, Shield, ArrowRight, Tag, ChevronUp, ChevronDown } from 'lucide-react';
...
const MobileStickyFooter: React.FC<MobileStickyFooterProps> = ({
  ...
  return (
    <div
      className={cn(
        'fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-2xl lg:hidden',
        'border-t-[3px] border-gray-900',
        'shadow-[0_-12px_30px_rgba(0,0,0,0.18)]',
    ...
    >
      ...
      <button
        ...
        className="w-full flex flex-col items-center pt-1.5 pb-0.5"
      >
        <span className="w-10 h-1 rounded-full bg-gray-300" />
        <span className="flex items-center gap-1 text-[10px] font-medium text-gray-500 mt-1">
          {expanded ? (
            <>Hide details <ChevronDown className="w-3 h-3" /></>
          ) : (
            <>Tap to expand <ChevronUp className="w-3 h-3" /></>
          )}
        </span>
      </button>
      ...
      {/* Expanded section */}
      {expanded && (
        <div className="px-4 pt-2 border-b border-gray-100 pb-3 animate-in slide-in-from-bottom-2 duration-200">
          <div className="flex items-center justify-between gap-2 mb-2.5">
            <span className="text-xs font-semibold text-gray-700">{planLabel}</span>
            <a
            ...
            >
              ...
              <span className="text-[9px] text-gray-500 ml-0.5">Trustpilot</span>
            </a>
          </div>

          <div className={cn('flex gap-2', isPulsing && 'animate-pulse')}>
            <Card type="monthly" title="Pay Monthly">
              ...
              <p className="text-[11px] font-semibold text-gray-900 mt-0.5">Over 12 months</p>
              <p className="text-[10px] text-gray-600">Equal to {dayLabel}/day</p>
            </Card>

            <Card type="full" title="Pay in Full">
              <div className="mt-1">
                <span className="text-xl font-extrabold text-[#0BA360]">£{discountedFull}</span>
              </div>
              <p className="text-[11px] font-semibold text-gray-900 mt-0.5">One payment</p>
              {displaySavings > 0 && (
      ...
      {/* Compact action bar */}
      <div className="px-4 pt-2 pb-[max(env(safe-area-inset-bottom),10px)]">
        <div className="flex items-center gap-3">
          {/* Left: plan + price */}
          <div className={cn('flex flex-col leading-tight min-w-0 flex-shrink', isPulsing && 'animate-pulse')}>
            <span className="text-[11px] font-semibold text-gray-500 truncate">{planLabel}</span>
            <div className="flex items-baseline gap-1">
              <span className="text-xl font-extrabold text-gray-900">£{selected === 'full' ? discountedFull : monthlyPrice}</span>
              <span className="text-[11px] text-gray-600">{selected === 'full' ? 'total' : '/mo'}</span>
            </div>
            {displaySavings > 0 && (
              <span className="text-[10px] font-semibold text-[#0BA360]">Save £{displaySavings} annually</span>
            )}
          </div>
