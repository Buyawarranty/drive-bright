...
export const GoogleAdsSettingsTab: React.FC = () => {
  ...
  // Date range helpers for agent filter
  const getDateRange = (range: string): { from: Date; to: Date } => {
    ...
    switch (range) {
      ...
      case 'last7': from = new Date(to); from.setDate(from.getDate() - 7); break;
      case 'last30': from = new Date(to); from.setDate(from.getDate() - 30); break;
      case 'thisMonth': from = new Date(now.getFullYear(), now.getMonth(), 1); break;
      case 'lastMonth': from = new Date(now.getFullYear(), now.getMonth() - 1, 1); to.setTime(new Date(now.getFullYear(), now.getMonth(), 0, 23, 59, 59).getTime()); break;
      case 'last90': from = new Date(to); from.setDate(from.getDate() - 90); break;
      case 'allTime': from = new Date(2024, 0, 1); break;
      default: from = new Date(to); from.setDate(from.getDate() - 30);
    }
    return { from, to };
  };
  ...
  const { data: agentSalesData, isLoading: agentSalesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch customers in date range
      const { data: customers } = await supabase
        .from('customers')
        .select('id, assigned_to, final_amount, created_at, signup_date')
        .eq('is_deleted', false)
        .in('status', ['active', 'Active'])
        .gte('signup_date', from.toISOString())
        .lte('signup_date', to.toISOString());

      // Fetch bumper in date range
      const { data: bumper } = await supabase
        .from('bumper_transactions')
      ...
      const agentMap = new Map<string, { name: string; email: string; role: string; deals: number; totalValue: number }>();
      ...
      // Unassigned bucket
      agentMap.set('unassigned', { name: 'Website (Unassigned)', email: '', role: 'website', deals: 0, totalValue: 0 });
      ...
      (customers || []).forEach(c => {
        ...
        if (agent) {
          agent.deals++;
          agent.totalValue += c.final_amount || 0;
        } else {
          // Agent not in active list, count under unassigned
          const unassigned = agentMap.get('unassigned')!;
          unassigned.deals++;
          unassigned.totalValue += c.final_amount || 0;
        }
      });

      // Bumper sales go to unassigned (no assigned_to on bumper)
      (bumper || []).forEach(b => {
        const unassigned = agentMap.get('unassigned')!;
        unassigned.deals++;
        unassigned.totalValue += b.final_amount || 0;
      });

      return Array.from(agentMap.entries())
        .map(([id, data]) => ({ id, ...data, avgValue: data.deals > 0 ? data.totalValue / data.deals : 0 }))
        .filter(a => a.deals > 0)
        .sort((a, b) => b.totalValue - a.totalValue);
    },
  });

  // Find highest month agent
  const highestAgent = agentSalesData && agentSalesData.length > 0 ? agentSalesData[0] : null;

  const filteredAgentSales = useMemo(() => {
    if (!agentSalesData) return [];
    if (agentFilter === 'all') return agentSalesData;
    return agentSalesData.filter(a => a.id === agentFilter);
  }, [agentSalesData, agentFilter]);

  const agentTotals = useMemo(() => {
    if (!filteredAgentSales) return { deals: 0, totalValue: 0, avgValue: 0 };
    const deals = filteredAgentSales.reduce((s, a) => s + a.deals, 0);
    const totalValue = filteredAgentSales.reduce((s, a) => s + a.totalValue, 0);
    return { deals, totalValue, avgValue: deals > 0 ? totalValue / deals : 0 };
  }, [filteredAgentSales]);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Breakdown of deals, total value, and average value per agent within a date range
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <div className="flex items-center gap-3 px-4 py-2 border rounded-lg bg-muted/30">
              {highestAgent && agentFilter === 'all' && (
                <Badge className="bg-amber-100 text-amber-800 border-amber-300">🏆 Highest: {highestAgent.name}</Badge>
              )}
              <span className="font-bold">£{agentTotals.totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
              <span className="text-sm text-muted-foreground">{agentTotals.deals} sales</span>
              <span className="text-sm text-muted-foreground">· Avg: £{agentTotals.avgValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {agentSalesLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No sales in this period</TableCell>
                  </TableRow>
                ) : filteredAgentSales.map(agent => (
                  <TableRow key={agent.id}>
                    <TableCell className="font-medium">{agent.name}</TableCell>
                    <TableCell>
                      <Badge variant="outline" className="text-xs capitalize">{agent.role.replace('_', ' ')}</Badge>
                    </TableCell>
                    <TableCell className="text-right font-semibold">{agent.deals}</TableCell>
                    <TableCell className="text-right font-semibold">£{agent.totalValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
                    <TableCell className="text-right text-muted-foreground">£{agent.avgValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
