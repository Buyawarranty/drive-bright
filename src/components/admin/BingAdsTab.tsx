...
export const BingAdsTab: React.FC = () => {
  ...
  const { data: bingPageViews, isLoading: pvLoading } = useQuery({
    queryKey: ['bing-page-views', rangeKey],
    queryFn: async () => {
  ...
  const { data: bingPaidCustomers, isLoading: paidLoading } = useQuery({
    queryKey: ['bing-paid-customers', rangeKey],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('customers')
  ...
  const { data: bingReconciliation } = useQuery({
    queryKey: ['bing-reconciliation', rangeKey],
    queryFn: async () => {
  ...
  }, [bingPageViews, bingLeads, bingPaidCustomers]);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Daily Bing funnel</CardTitle>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Top pages (Bing traffic)</CardTitle>
          <CardDescription>Which pages Bing visitors are landing on</CardDescription>
        </CardHeader>
        <CardContent>
