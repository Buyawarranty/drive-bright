...
export const NewLeadsTab: React.FC<NewLeadsTabProps> = ({
  ...
  const WORKED_STATUSES_NO_BUBBLE = ['lost', 'not_interested', 'contacted', 'follow_up', 'converted', 'fake_lead', 'callback', 'quoted', 'not_eligible', 'unsubscribed'];
  ...
  const leadCounts = useMemo(() => {
    ...
    const liveCount = countScopedLeads.filter(
      l => l.status !== 'lost' && l.status !== 'fake_lead' && l.status !== 'not_eligible' && (l.status as string) !== 'unsubscribed' && (l.status as string) !== 'archived'
    ).length;
    ...
    const live = {
      ...
      high_priority: countScopedLeads.filter(
        l =>
          ...
          l.status !== 'lost' &&
          l.status !== 'fake_lead' &&
          l.status !== 'not_eligible' && (l.status as string) !== 'unsubscribed' &&
  ...
  const agentLiveLeadCounts = useMemo(() => {
    const counts: Record<string, number> = { unassigned: 0 };
    teamScopedAgentCountRows.forEach(row => {
      if (row.status === 'lost' || row.status === 'fake_lead' || row.status === 'not_eligible' || (row.status as string) === 'unsubscribed' || row.status === 'archived') return;
      if (!row.assigned_to) {
        counts.unassigned = (counts.unassigned || 0) + 1;
      } else {
        counts[row.assigned_to] = (counts[row.assigned_to] || 0) + 1;
