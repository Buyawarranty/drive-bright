...
const LeadAssignmentStream: React.FC<Props> = ({ agents, teamNameByAgent, canReassign = false }) => {
  ...
  const { repeatByLeadId } = useRepeatCustomers(
    useMemo(() => rows.map(r => ({ id: r.id, email: (r as any).email, vehicle_reg: r.vehicle_reg, phone: (r as any).phone, first_name: (r as any).first_name, last_name: (r as any).last_name, created_at: (r as any).created_at })), [rows])
  );
