...
const HelperCallout: React.FC<Props> = ({ topic, className, defaultOpen = false }) => {
  ...
  return (
    <div
      className={cn(
        'mt-3 rounded-xl border border-blue-200 bg-blue-50/60 overflow-hidden',
        className
      )}
    >
      <button
      ...
      >
        ...
        <span className="flex-1 min-w-0">
          <span className="block text-[13px] md:text-sm font-semibold text-blue-900 leading-tight">
            {c.title}
          </span>
          <span className="block text-[11px] md:text-xs text-blue-800/80 mt-0.5 truncate">
            Plain-English explanation with a real example.
          </span>
        </span>
        <ChevronDown
          className={cn(
            'w-4 h-4 text-blue-700 flex-shrink-0 transition-transform',
            open && 'rotate-180'
          )}
        />
      </button>
      ...
      {open && (
        <div className="px-3 pb-3 pt-1 border-t border-blue-200/70 bg-white/70 space-y-2.5">
          <p className="text-[13px] text-foreground/90 leading-snug">{c.short}</p>

          <div className="rounded-lg bg-blue-50 border border-blue-100 px-3 py-2 text-[12px] text-blue-900 leading-snug">
          ...
          <div className="flex items-start gap-2 rounded-lg bg-emerald-50 border border-emerald-200 px-3 py-2">
            ...
            <p className="text-[12px] leading-snug text-emerald-900 font-medium">{c.takeaway}</p>
          </div>
        </div>
      )}
    </div>
