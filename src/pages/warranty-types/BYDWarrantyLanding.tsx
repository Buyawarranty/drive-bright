...
import heroImage from '@/assets/byd-extended-warranty-uk.png';
...
const brandName = 'BYD';
const brandSlug = 'byd-warranty';
const brandMake = 'BYD';
const startingPrice = '24';

const modelCategories = {
  'Electric SUVs': { 'Atto 3': [''], 'Tang': ['EV600'], 'Yuan Plus': [''] },
  'Electric Sedans': { 'Seal': [''], 'Han': ['EV'], 'Dolphin': [''] },
  'Plug-in Hybrid': { 'Song Plus': ['DM-i'], 'Tang': ['DM-p'] },
};

type ModelCategory = keyof typeof modelCategories;

const faqs = [
  { question: "Is a BYD extended warranty worth it?", answer: "Yes. BYD vehicles use advanced Blade Battery and electric drivetrain technology. While reliable, EV-specific repairs like battery modules or motor replacements can cost £3,000–£10,000." },
  { question: "How much does a BYD extended warranty cost?", answer: `BYD warranty prices start from £${startingPrice}/month depending on model, mileage, and claim limit.` },
  { question: "Does it cover the Blade Battery?", answer: "Yes, our comprehensive plan covers the battery management system, battery modules, thermal management, and charging components." },
  { question: "Can I use my own garage?", answer: "Yes, any VAT-registered garage in the UK, including EV specialists and independent garages." },
  { question: "Is the electric motor covered?", answer: "Yes, the electric drive motor, inverter, reduction gearbox, and all associated electronics are fully covered." },
...
const testimonials = [
  { name: "Rachel M.", location: "London", model: "BYD Atto 3", text: "Charging system issue resolved quickly. Great to have warranty cover for my new BYD. Highly recommend.", rating: 5 },
  { name: "Daniel K.", location: "Bristol", model: "BYD Seal", text: "Infotainment system replaced under warranty. Fast, professional service. Peace of mind for EV owners.", rating: 5 },
  { name: "Emma W.", location: "Manchester", model: "BYD Dolphin", text: "Air conditioning compressor covered. £1,200 saved. Essential cover for any BYD owner.", rating: 5 },
  { name: "Mark S.", location: "Birmingham", model: "BYD Tang", text: "Suspension sensor replaced. No hassle claim process. Very happy with the cover.", rating: 5 }
];

const repairs = [
  { name: 'Battery Module Replacement', cost: '£3,000 – £10,000', icon: Battery, severity: 'critical' as const },
  { name: 'Electric Motor', cost: '£2,500 – £6,000', icon: Zap, severity: 'critical' as const },
  { name: 'Inverter/Power Electronics', cost: '£2,000 – £4,500', icon: Zap, severity: 'critical' as const },
  { name: 'Onboard Charger', cost: '£1,000 – £2,500', icon: Battery, severity: 'high' as const },
  { name: 'Battery Management System', cost: '£1,500 – £3,000', icon: Zap, severity: 'high' as const },
  { name: 'Thermal Management System', cost: '£1,200 – £2,800', icon: Wrench, severity: 'high' as const },
  { name: 'Drive Unit Reduction Gear', cost: '£1,500 – £3,500', icon: Wrench, severity: 'high' as const },
  { name: 'ECU Replacement', cost: '£800 – £2,000', icon: Zap, severity: 'medium' as const },
  { name: 'Air Conditioning', cost: '£600 – £1,500', icon: Car, severity: 'medium' as const },
];

const BYDWarrantyLanding: React.FC = () => {
    ...
    <>
      ...
      <main className="min-h-screen bg-white" role="main">
            ...
            <div className="grid lg:grid-cols-2 gap-6 lg:gap-12 items-center">
              <div className="text-center lg:text-left">
                ...
                <div className="mx-auto lg:mx-0 space-y-4">
                  ...
                  <p className="text-xs text-gray-500 mt-3 text-center lg:text-left">{brandName} is a registered trademark of BYD Company Ltd. We are an independent warranty provider.</p>
                  <TrustCallbackPanel />
                </div>
              </div>
              <div className="relative">
                ...
                <div className="flex flex-col items-center gap-4 mt-6">
                  <div className="flex items-center justify-center gap-3 sm:gap-4 lg:gap-6 flex-wrap">
                    <div className="flex items-center space-x-1.5"><Battery className="w-4 h-4 sm:w-5 sm:h-5 text-green-500" /><span className="font-medium text-gray-700 text-xs sm:text-sm lg:text-base">Electric</span></div>
                    ...
                    <div className="flex items-center space-x-1.5"><Zap className="w-4 h-4 sm:w-5 sm:h-5 text-green-500" /><span className="font-medium text-gray-700 text-xs sm:text-sm lg:text-base">PHEV</span></div>
                  </div>
        ...
        <section className="py-10 md:py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-8 md:mb-12">
              ...
              <p className="text-base md:text-lg text-gray-600 max-w-2xl mx-auto">EV-specific components like batteries and motors can be extremely expensive to replace.</p>
            </div>
            ...
            <div className="mt-8 md:mt-12 text-center">
              <div className="bg-brand-deep-blue rounded-2xl p-8 md:p-12 max-w-3xl mx-auto">
                <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 mb-4">
                  <div className="text-center"><p className="text-xs md:text-sm font-semibold text-white/60 uppercase tracking-wider mb-1">Typical repair</p><p className="text-2xl md:text-4xl font-bold text-red-400 line-through decoration-2">£4,000</p></div>
        ...
        <section className="py-12 md:py-20 bg-gradient-to-b from-slate-50 to-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-8 md:mb-12"><h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 mb-3">All {brandName} Models Covered</h2></div>
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2 md:gap-3">
              {filteredModels.map(({ model, generations }) => (
                <button key={model} onClick={() => { setSelectedModel(model); scrollToQuoteForm(); }} className={`group bg-white rounded-lg px-2 py-3 text-center border transition-all ${selectedModel === model ? 'border-blue-500 ring-1 ring-blue-500/20 shadow-md' : 'border-slate-200 hover:border-slate-300 hover:shadow-sm'}`}>
                  <Battery className={`w-6 h-6 mx-auto mb-1.5 ${selectedModel === model ? 'text-blue-500' : 'text-slate-400'}`} strokeWidth={1.25} />
        ...
        <section className="py-10 md:py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {[{ icon: Shield, title: 'Comprehensive Cover', desc: 'Battery, motor, electronics & more' }, { icon: MapPin, title: 'Any UK Garage', desc: 'Any VAT-registered garage' }, { icon: Phone, title: 'UK Claims Team', desc: 'Fast, hassle-free claims' }, { icon: Clock, title: 'Instant Cover', desc: 'Protection starts immediately' }].map((item, i) => (
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div className="text-center md:text-left">
                ...
                <ul className="space-y-2 text-left max-w-md mx-auto md:mx-0">
                  {['Use any VAT-registered garage', 'EV specialist or independent', 'Recovery to nearest garage', 'UK-based claims team'].map((item, i) => (<li key={i} className="flex items-center gap-2"><Check className="w-5 h-5 text-green-400 flex-shrink-0" /><span>{item}</span></li>))}
                </ul>
              </div>
        ...
        <RelatedInternalLinks
          links={[
            { to: '/warranty-types/cadillac-warranty/', label: 'cadillac extended warranty' },
...
export default BYDWarrantyLanding;
