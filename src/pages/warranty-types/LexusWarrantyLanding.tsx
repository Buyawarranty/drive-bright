...
import heroImage from '@/assets/lexus-extended-warranty-uk.png';
...
const brandSlug = 'lexus-warranty';
const brandMake = 'Lexus';
const startingPrice = '24';

const modelCategories = {
  'SUVs': { 'NX': ['AZ10', 'AZ20'], 'RX': ['AL20', 'AL30'], 'UX': ['MA10'], 'RZ': [''], 'LX': ['J200', 'J300'] },
  'Sedans': { 'IS': ['XE30', 'XE35'], 'ES': ['XV70'], 'LS': ['XF50'] },
  'Coupes & Convertibles': { 'LC': ['URZ100'], 'RC': ['ASC10'], 'RC F': ['USC10'] },
  'Hybrid & Electric': { 'NX Hybrid': ['AZ20h'], 'RX Hybrid': ['AL30h'], 'UX 300e': ['MA10 EV'], 'RZ 450e': [''] },
};
type ModelCategory = keyof typeof modelCategories;

const faqs = [
  { question: "Is a Lexus extended warranty worth it?", answer: "Yes. While Lexus vehicles are known for reliability, when things do go wrong, repairs use premium parts and can be very expensive. Our warranty provides peace of mind against unexpected costs." },
  { question: "How much does a Lexus warranty cost?", answer: `From £${startingPrice}/month depending on model, mileage, and claim limit. Flexible monthly or annual payments.` },
  { question: "Can I buy cover after the Lexus manufacturer warranty expires?", answer: "Yes, we cover vehicles up to 150,000 miles and 15 years old, even if your manufacturer warranty has ended." },
  { question: "Does it cover Lexus hybrid models?", answer: "Yes, our comprehensive plan covers hybrid components including the hybrid battery, electric motor, power control unit, and inverter." },
  { question: "Can I use my own garage?", answer: "Yes, any VAT-registered garage in the UK — you're not restricted to Lexus dealers." },
  { question: "How do I make a claim?", answer: "Call our UK claims team or submit online. We authorise quickly and settle the bill directly with your chosen garage." },
];

const testimonials = [
  { name: "Andrew C.", location: "Surrey", model: "Lexus RX", text: "Air suspension issue covered in full — £2,800 claim paid quickly. Outstanding service throughout.", rating: 5 },
  { name: "Jennifer M.", location: "London", model: "Lexus NX Hybrid", text: "Hybrid battery cooling pump failed. Covered completely. Without warranty it would have been £1,500.", rating: 5 },
  { name: "Paul D.", location: "Edinburgh", model: "Lexus IS", text: "Gearbox issue sorted within a week. £3,200 claim with no hassle. Highly recommend.", rating: 5 },
  { name: "Emma W.", location: "Manchester", model: "Lexus LC", text: "Fantastic cover for my LC. ECU issue resolved quickly and professionally. Worth every penny.", rating: 5 }
];

const repairs = [
  { name: 'Hybrid Battery Pack', cost: '£2,500 – £6,000', icon: Battery, severity: 'critical' as const },
  { name: 'Automatic Gearbox', cost: '£2,500 – £5,500', icon: Wrench, severity: 'critical' as const },
  { name: 'Engine Rebuild', cost: '£3,000 – £6,000', icon: Car, severity: 'critical' as const },
  { name: 'Air Suspension System', cost: '£1,500 – £3,500', icon: Car, severity: 'high' as const },
  { name: 'Power Control Unit', cost: '£1,800 – £3,500', icon: Zap, severity: 'high' as const },
  { name: 'ECU Replacement', cost: '£900 – £2,200', icon: Zap, severity: 'medium' as const },
  { name: 'Turbocharger', cost: '£1,500 – £3,000', icon: Wrench, severity: 'high' as const },
  { name: 'Fuel Injector Set', cost: '£800 – £2,000', icon: Zap, severity: 'medium' as const },
  { name: 'Power Steering Rack', cost: '£900 – £2,000', icon: Wrench, severity: 'medium' as const },
];

const LexusWarrantyLanding: React.FC = () => {
  ...
  const handleGetQuote = async (mileageOverride?: string) => {
    ...
    if (!effectiveMileage) { toast({ title: "Mileage Required", description: "Please select your mileage.", variant: "destructive" }); return; }
    setIsLookingUp(true);
    try {
      ...
      const vd = { regNumber, mileage: effectiveMileage, make: data?.found ? (data.make || brandMake) : undefined, model: data?.found ? data.model : undefined, fuelType: data?.found ? data.fuelType : undefined, transmission: data?.found ? data.transmission : undefined, year: data?.found ? data.yearOfManufacture : undefined, vehicleType: 'car', manufactureDate: data?.found ? data.manufactureDate : undefined };
  ...
  const productSchema = { "@context": "https://schema.org", "@type": "Product",
      "image": ["https://buyawarranty.co.uk/extended_warranty_uk-car-trustworthy-reviews.png"], "name": `${brandName} Extended Warranty UK`, "description": `Comprehensive warranty for all ${brandName} models including NX, RX, IS, ES, LC, and hybrid models.`, "brand": { "@type": "Brand", "name": "Buy A Warranty" }, "offers": { "@type": "Offer", "priceCurrency": "GBP", "price": startingPrice, "availability": "https://schema.org/InStock", "url": `https://buyawarranty.co.uk/warranty-types/${brandSlug}/` }, "aggregateRating": { "@type": "AggregateRating", "ratingValue": TRUSTPILOT_RATING_VALUE, "reviewCount": TRUSTPILOT_REVIEW_COUNT } };
    ...
    <>
      ...
      <main className="min-h-screen bg-white" role="main">
            ...
            <div className="grid lg:grid-cols-2 gap-6 lg:gap-12 items-center">
              <div className="text-center lg:text-left">
                ...
                <div className="mx-auto lg:mx-0 space-y-4">
                  ...
                  <p className="text-xs text-gray-500 mt-3 text-center lg:text-left">{brandName} is a registered trademark of Toyota Motor Corporation. We are an independent warranty provider.</p>
                  <TrustCallbackPanel />
                </div>
              </div>
        ...
        <section className="py-10 md:py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-8 md:mb-12">
              <div className="inline-flex items-center gap-2 bg-red-50 px-3 py-1.5 rounded-full mb-3"><Wrench className="w-4 h-4 md:w-5 md:h-5 text-red-600" /><span className="text-xs md:text-sm font-semibold text-red-700">Without warranty, you pay the full bill</span></div>
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-3">What {brandName} repairs actually cost</h2>
              <p className="text-base md:text-lg text-gray-600 max-w-2xl mx-auto">{brandName} uses premium parts — one repair could cost thousands.</p>
            </div>
            ...
            <div className="mt-8 md:mt-12 text-center">
              <div className="bg-brand-deep-blue rounded-2xl p-8 md:p-12 max-w-3xl mx-auto">
                <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 mb-4">
                  <div className="text-center"><p className="text-xs md:text-sm font-semibold text-white/60 uppercase mb-1">Typical repair</p><p className="text-2xl md:text-4xl font-bold text-red-400 line-through decoration-2">£3,000</p></div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-2 md:gap-3">
              {filteredModels.map(({ model, generations, category }) => {
                const isHybridEV = category === 'Hybrid & Electric';
                return (
                  <button key={model} onClick={() => { setSelectedModel(model); scrollToQuoteForm(); }} className={`group bg-white rounded-lg px-2 py-3 text-center border transition-all ${selectedModel === model ? 'border-blue-500 ring-1 ring-blue-500/20 shadow-md' : 'border-slate-200 hover:shadow-sm'}`}>
                    {isHybridEV && <div className="absolute -top-1.5 -right-1.5 bg-blue-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-full flex items-center gap-0.5"><Zap className="w-2 h-2" />EV</div>}
        ...
        <section className="py-10 md:py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {[{ icon: Shield, title: 'Comprehensive Cover', desc: 'Engine, gearbox, hybrid & more' }, { icon: MapPin, title: 'Any UK Garage', desc: 'Any VAT-registered garage' }, { icon: Phone, title: 'UK Claims Team', desc: 'Fast, hassle-free claims' }, { icon: Clock, title: 'Instant Cover', desc: 'Protection starts immediately' }].map((item, i) => (
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div className="text-center md:text-left">
                ...
                <ul className="space-y-2 text-left max-w-md mx-auto md:mx-0">
                  {['Use any VAT-registered garage', `${brandName} dealer or independent`, 'Recovery to nearest garage', 'UK-based claims team'].map((item, i) => (<li key={i} className="flex items-center gap-2"><Check className="w-5 h-5 text-green-400 flex-shrink-0" /><span>{item}</span></li>))}
                </ul>
              </div>
        ...
        <RelatedInternalLinks
          links={[
            { to: '/warranty-types/mg-warranty/', label: 'mg extended warranty' },
...
export default LexusWarrantyLanding;
