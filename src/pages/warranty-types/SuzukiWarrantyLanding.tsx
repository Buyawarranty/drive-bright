...
import heroImage from '@/assets/suzuki-extended-warranty-uk.png';
...
const brandName = 'Suzuki';
const brandSlug = 'suzuki-warranty';
const brandMake = 'SUZUKI';
const startingPrice = '19';

const modelCategories = {
  'Hatchbacks': { 'Swift': ['ZC33S', 'ZC83S'], 'Ignis': ['MF'], 'Baleno': ['EW'] },
  'SUVs & Crossovers': { 'Vitara': ['LY'], 'S-Cross': ['JY'], 'Jimny': ['JB74'] },
  'Hybrid': { 'Swift Hybrid': ['ZC33S'], 'Vitara Hybrid': ['LY'], 'S-Cross Hybrid': ['JY'] },
};

type ModelCategory = keyof typeof modelCategories;

const faqs = [
  { question: "Is a Suzuki extended warranty worth it?", answer: "Yes. While Suzukis are reliable, unexpected repairs like CVT gearbox or turbo failures can cost £1,500–£3,500. Our warranty gives you peace of mind from just £19/month." },
  { question: "How much does a Suzuki extended warranty cost?", answer: `Suzuki warranty prices start from £${startingPrice}/month depending on model, mileage, and claim limit. Flexible monthly or annual payments available.` },
  { question: "Can I get cover after my Suzuki dealer warranty expires?", answer: "Yes, we cover vehicles up to 150,000 miles and 15 years old, even after manufacturer warranty has expired." },
  { question: "Can I use my own garage?", answer: "Yes, any VAT-registered garage in the UK, including Suzuki specialists and independent garages." },
  { question: "Is the Suzuki Jimny covered?", answer: "Yes, we cover all Suzuki models including the Jimny, with full protection for the 4WD system, transfer case, and all mechanical components." },
  { question: "Are hybrid Suzuki models covered?", answer: "Yes, all Suzuki mild hybrid and self-charging hybrid systems are fully covered including the electric motor, battery management, and power electronics." },
...
const testimonials = [
  { name: "James W.", location: "Leeds", model: "Suzuki Vitara", text: "CVT gearbox issue fully covered — would have been £2,200 out of pocket. Excellent service!", rating: 5 },
  { name: "Helen M.", location: "Cardiff", model: "Suzuki Swift", text: "Catalytic converter replacement sorted quickly. £1,400 claim paid without fuss. Highly recommend.", rating: 5 },
  { name: "Mark B.", location: "Glasgow", model: "Suzuki Jimny", text: "Transfer case problem on my Jimny — fully covered. The claims process was brilliant.", rating: 5 },
  { name: "Rachel D.", location: "Brighton", model: "Suzuki S-Cross", text: "Turbo failure covered completely. Saved me over £2,000. Worth every penny of the monthly cost.", rating: 5 }
];

const repairs = [
  { name: 'CVT Gearbox Replacement', cost: '£1,800 – £3,500', icon: Wrench, severity: 'critical' as const },
  { name: 'Turbocharger Replacement', cost: '£1,200 – £2,800', icon: Zap, severity: 'critical' as const },
  { name: 'Catalytic Converter', cost: '£1,000 – £2,200', icon: Wrench, severity: 'high' as const },
  { name: '4WD Transfer Case', cost: '£800 – £2,000', icon: Car, severity: 'high' as const },
  { name: 'Power Steering Pump', cost: '£600 – £1,400', icon: Wrench, severity: 'medium' as const },
  { name: 'ECU Replacement', cost: '£700 – £1,800', icon: Zap, severity: 'high' as const },
  { name: 'Fuel Injector Set', cost: '£500 – £1,200', icon: Zap, severity: 'medium' as const },
  { name: 'Hybrid Battery Module', cost: '£1,500 – £3,500', icon: Battery, severity: 'critical' as const },
  { name: 'Clutch & Flywheel', cost: '£800 – £1,800', icon: Wrench, severity: 'high' as const },
];

const SuzukiWarrantyLanding: React.FC = () => {
    ...
    <>
      ...
      <main className="min-h-screen bg-white" role="main">
            ...
            <div className="grid lg:grid-cols-2 gap-6 lg:gap-12 items-center">
              <div className="text-center lg:text-left">
                ...
                <div className="mx-auto lg:mx-0 space-y-4">
                  ...
                  <p className="text-xs text-gray-500 mt-3 text-center lg:text-left">{brandName} is a registered trademark of Suzuki Motor Corporation. We are an independent warranty provider.</p>
                  <TrustCallbackPanel />
                </div>
              </div>
              <div className="relative">
                ...
                <div className="flex flex-col items-center gap-4 mt-6">
                  <div className="flex items-center justify-center gap-3 sm:gap-4 lg:gap-6 flex-wrap">
                    ...
                    <div className="flex items-center space-x-1.5"><Battery className="w-4 h-4 sm:w-5 sm:h-5 text-green-500" /><span className="font-medium text-gray-700 text-xs sm:text-sm lg:text-base">Hybrid</span></div>
                  </div>
            ...
            <div className="mt-8 md:mt-12 text-center">
              <div className="bg-brand-deep-blue rounded-2xl p-8 md:p-12 max-w-3xl mx-auto">
                <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 mb-4">
                  <div className="text-center"><p className="text-xs md:text-sm font-semibold text-white/60 uppercase tracking-wider mb-1">Typical repair</p><p className="text-2xl md:text-4xl font-bold text-red-400 line-through decoration-2">£2,200</p></div>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div className="text-center md:text-left">
                ...
                <ul className="space-y-2 text-left max-w-md mx-auto md:mx-0">
                  {['Use any VAT-registered garage', 'Suzuki specialist or independent', 'Recovery to nearest garage', 'UK-based claims team'].map((item, i) => (<li key={i} className="flex items-center gap-2"><Check className="w-5 h-5 text-green-400 flex-shrink-0" /><span>{item}</span></li>))}
                </ul>
              </div>
        ...
        <RelatedInternalLinks
          links={[
            { to: '/warranty-types/tesla-warranty/', label: 'tesla extended warranty uk' },
...
export default SuzukiWarrantyLanding;
