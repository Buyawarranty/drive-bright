  ...
  return (
    <>
      <Helmet>
        ...
        <link rel="canonical" href="https://buyawarranty.co.uk/warranty-types/nissan/" />
        ...
        <meta property="og:url" content="https://buyawarranty.co.uk/warranty-types/nissan/" />
      ...
      <main itemScope itemType="https://schema.org/WebPage">
        <section id="hero-section" className="relative bg-gradient-to-br from-red-50 via-white to-gray-50 pt-6 pb-10 md:pt-10 md:pb-16 overflow-hidden">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
              <div className="space-y-5">
                <div className="inline-flex items-center gap-2 bg-red-100 text-red-700 px-4 py-2 rounded-full text-sm font-medium"><Shield className="h-4 w-4" /> Official Nissan Extended Warranty Partner</div>
                <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-gray-900 leading-tight">Nissan Extended <span className="text-brand-orange">Warranty</span> UK</h1>
                <p className="text-base md:text-lg text-gray-600 max-w-xl">Protect your Nissan with comprehensive cover from just <strong>£20/month</strong>. All models from 2012 to 2026 including Qashqai, Juke, X-Trail, Leaf & Ariya. CVT gearbox covered. Use any UK garage.</p>
                <div className="bg-white rounded-2xl shadow-xl p-5 md:p-6 border border-gray-100">
                  <div className="space-y-4">
                    <div><label htmlFor="reg-input" className="block text-sm font-semibold text-gray-700 mb-1.5">Enter Your Registration</label><div className="relative"><div className="absolute left-0 top-0 bottom-0 w-12 bg-blue-600 rounded-l-lg flex items-center justify-center"><span className="text-white text-xs font-bold">GB</span></div><input id="reg-input" type="text" value={regNumber} onChange={handleRegChange} placeholder="AB12 CDE" className="w-full pl-16 pr-4 py-3.5 text-xl font-bold uppercase tracking-wider bg-yellow-50 border-2 border-yellow-400 rounded-lg focus:border-yellow-500 focus:ring-2 focus:ring-yellow-200 text-center" maxLength={8} aria-label="Vehicle registration number" /></div></div>
                    ...
                    <Button onClick={handleGetQuote} disabled={isLookingUp} className="w-full bg-brand-orange hover:bg-brand-orange/90 text-white font-bold py-4 text-lg rounded-xl shadow-lg animate-cta-enhanced" size="lg">{isLookingUp ? <span className="flex items-center gap-2"><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>Looking up vehicle...</span> : <span className="flex items-center gap-2">Get My Free Quote <ArrowRight className="h-5 w-5" /></span>}</Button>
                    <div className="flex items-center justify-center gap-4 text-xs text-gray-500"><span className="flex items-center gap-1"><Check className="h-3 w-3 text-green-500" /> No obligation</span><span className="flex items-center gap-1"><Check className="h-3 w-3 text-green-500" /> Instant quote</span><span className="flex items-center gap-1"><Check className="h-3 w-3 text-green-500" /> Cancel anytime</span></div>
                  </div>
                </div>
                <TrustCallbackPanel />
                <div className="flex items-center gap-3"><OptimizedImage src={trustpilotExcellent} alt="Trustpilot Excellent Rating" className="h-10" width={120} height={40} /><span className="text-sm text-gray-600">Rated <strong>Excellent</strong> by Nissan owners</span></div>
              </div>
              <div className="relative hidden lg:flex flex-col items-center justify-center">
                <div className="relative flex items-end justify-center gap-4">
                  <OptimizedImage src={nissanQashqaiHero} alt="Nissan Qashqai extended warranty UK" className="max-w-[35%] h-auto drop-shadow-2xl" priority width={269} height={179} />
                  <OptimizedImage src={pandaThumbsUp} alt="Buy A Warranty mascot" className="max-w-[25%] h-auto" priority width={192} height={192} />
                  <OptimizedImage src={nissanJukeWarranty} alt="Nissan Juke used car warranty UK" className="max-w-[35%] h-auto drop-shadow-2xl" priority width={269} height={179} />
                </div>
                <div className="mt-6 flex items-center gap-2"><div className="flex -space-x-1">{[...Array(5)].map((_, i) => <Star key={i} className="h-5 w-5 fill-yellow-400 text-yellow-400" />)}</div><span className="text-sm font-medium text-gray-700">Trusted by thousands of Nissan owners</span></div>
              </div>
            </div>
          </div>
        </section>
