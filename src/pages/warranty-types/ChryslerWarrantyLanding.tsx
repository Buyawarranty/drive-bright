...
import heroImage from '@/assets/chrysler-extended-warranty-uk.png';
...
const modelCategories = {
  'Sedans': { '300C': ['LX', 'LX2'], '300': ['LX'] },
  'MPVs': { 'Grand Voyager': ['RT'], 'Voyager': ['RG', 'RT'], 'Ypsilon': [''] },
  'SUVs': { 'Pacifica': ['RU'] },
  'Crossovers': { 'Delta': [''], 'PT Cruiser': ['PT'] },
};

type ModelCategory = keyof typeof modelCategories;

const faqs = [
  { question: "Is a Chrysler extended warranty worth it in the UK?", answer: "Yes. Chrysler vehicles use American-spec components that can be expensive to source in the UK. A transmission or engine failure can cost £3,000–£7,000." },
  { question: "How much does a Chrysler extended warranty cost?", answer: `Chrysler warranty prices start from £${startingPrice}/month depending on model, mileage, and claim limit.` },
  ...
  { question: "Is the 300C HEMI covered?", answer: "Yes, the HEMI V8 engine and all variants are fully covered including the complete engine internals and ancillaries." },
  { question: "Are Grand Voyager models covered?", answer: "Yes, all Chrysler Grand Voyager and Voyager models are covered including the sliding door mechanisms and power systems." },
...
const testimonials = [
  { name: "Robert J.", location: "Essex", model: "Chrysler 300C", text: "Gearbox issue sorted under warranty — £3,200 saved. Essential cover for any 300C owner.", rating: 5 },
  { name: "Karen H.", location: "Manchester", model: "Chrysler Grand Voyager", text: "Power sliding door motor replaced. Fast claim, no fuss. Great warranty for families.", rating: 5 },
  { name: "Neil P.", location: "London", model: "Chrysler 300", text: "Engine sensor failure covered completely. Professional service throughout. Highly recommend.", rating: 5 },
  { name: "Sandra M.", location: "Bristol", model: "Chrysler Voyager", text: "AC compressor replaced under warranty. Would have been £1,400 out of pocket. Worth every penny.", rating: 5 }
];

const repairs = [
  { name: 'HEMI Engine Rebuild', cost: '£3,500 – £8,000', icon: Wrench, severity: 'critical' as const },
  { name: 'Automatic Transmission', cost: '£2,500 – £5,500', icon: Car, severity: 'critical' as const },
  { name: 'Power Sliding Door System', cost: '£800 – £2,000', icon: Wrench, severity: 'high' as const },
  { name: 'ECU Replacement', cost: '£1,000 – £2,500', icon: Zap, severity: 'high' as const },
  { name: 'Turbo/Supercharger', cost: '£1,500 – £3,500', icon: Zap, severity: 'high' as const },
  { name: 'Power Steering Rack', cost: '£900 – £2,000', icon: Wrench, severity: 'medium' as const },
  { name: 'Fuel Injector Set', cost: '£800 – £1,800', icon: Zap, severity: 'medium' as const },
  { name: 'Air Conditioning', cost: '£600 – £1,500', icon: Car, severity: 'medium' as const },
  { name: 'Transfer Case (AWD)', cost: '£1,200 – £2,800', icon: Truck, severity: 'high' as const },
];

const ChryslerWarrantyLanding: React.FC = () => {
              ...
              <div className="relative">
                ...
                <div className="flex flex-col items-center gap-4 mt-6">
                  <div className="flex items-center justify-center gap-3 sm:gap-4 lg:gap-6 flex-wrap">
                    <div className="flex items-center space-x-1.5"><Car className="w-4 h-4 sm:w-5 sm:h-5 text-green-500" /><span className="font-medium text-gray-700 text-xs sm:text-sm lg:text-base">Sedans</span></div>
                    <div className="flex items-center space-x-1.5"><Truck className="w-4 h-4 sm:w-5 sm:h-5 text-green-500" /><span className="font-medium text-gray-700 text-xs sm:text-sm lg:text-base">MPVs</span></div>
            ...
            <div className="mt-8 md:mt-12 text-center">
              <div className="bg-brand-deep-blue rounded-2xl p-8 md:p-12 max-w-3xl mx-auto">
                <div className="flex flex-col md:flex-row items-center justify-center gap-4 md:gap-8 mb-4">
                  <div className="text-center"><p className="text-xs md:text-sm font-semibold text-white/60 uppercase tracking-wider mb-1">Typical repair</p><p className="text-2xl md:text-4xl font-bold text-red-400 line-through decoration-2">£3,500</p></div>
            ...
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div className="text-center md:text-left">
                ...
                <ul className="space-y-2 text-left max-w-md mx-auto md:mx-0">
                  {['Use any VAT-registered garage', 'Chrysler specialist or independent', 'Recovery to nearest garage', 'UK-based claims team'].map((item, i) => (<li key={i} className="flex items-center gap-2"><Check className="w-5 h-5 text-green-400 flex-shrink-0" /><span>{item}</span></li>))}
                </ul>
              </div>
...
export default ChryslerWarrantyLanding;
